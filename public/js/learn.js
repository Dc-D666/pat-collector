...
Views.learnList = async () => {
  ...
  view.innerHTML = `
    <div class="page">
      <div class="page-head">
        <h1 class="page-title">学AI</h1>
        <div class="page-sub">走进 AI、学会用 AI —— 面向零基础同学的系列教程</div>
      </div>
  ...
  content.innerHTML = chapters.map((c, ci) => `
    <div class="card learn-chapter">
      <div class="learn-chapter-head">
        <span class="learn-chapter-no">第 ${c.chapter} 章</span>
        <span class="learn-chapter-count">${c.articles.length} 篇</span>
      </div>
      <div class="learn-list">
        ${c.articles.map((a, ai) => `
          <a class="learn-item" href="#/learn/${encodeURIComponent(a.slug)}">
            <span class="learn-item-no">${ci}.${ai + 1}</span>
...
async function loadTaskProgress(articleId) {
  try {
    ...
    document.querySelectorAll('.learn-task[data-article="' + articleId + '"]').forEach((task) => {
      ...
      task.dataset.done = '1';
      const doneBtn = task.querySelector('.task-done-btn');
