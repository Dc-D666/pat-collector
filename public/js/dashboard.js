...
Views.files = () => {
  ...
  view.innerHTML = `
    <div class="page">
      ...
      <div id="panel-apps" style="display:${isQqBound ? '' : 'none'};">
        <!-- ✨ AI 轻应用入口卡（2026-08-27：创作功能已搬至独立页 #/gen，此处仅留入口） -->
        <div class="card" id="gen-app-card" style="margin-bottom:14px;">
          ...
          <div style="font-size:12px;color:var(--text-dim);margin-bottom:12px;line-height:1.7;">描述你的想法，AI 生成一个能玩的小程序，制作并提交你的作品（AI 小学堂第2章配套）。</div>
          <button class="btn btn-primary" id="gen-enter-btn" type="button">进入 AI 轻应用</button>
        </div>
  ...
  // ✨ 进入 AI 轻应用独立页（#/gen）入口
  const genEnterBtn = document.getElementById('gen-enter-btn');
  if (genEnterBtn) genEnterBtn.onclick = () => { location.hash = '#/gen'; };
  ...
  async function loadFiles() {
    ...
    if (!list) return;
    let files;
    // 站内生成的 AI 轻应用（source='gen'）归「AI 轻应用」独立页 #/gen，不混入此处的项目文件列表
    try {
